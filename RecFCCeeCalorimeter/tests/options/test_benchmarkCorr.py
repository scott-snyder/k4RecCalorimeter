...
calibEcalBarrel = CalibrateInLayersTool(
    "CalibrateECalBarrel",
    samplingFraction=[0.3864252122990472] * 1
    + [0.13597644835735828] * 1
    + [0.14520427829645913] * 1
    + [0.1510076084632846] * 1
    + [0.1552347580991012] * 1
    + [0.159694330729184] * 1
    + [0.1632954482794191] * 1
    + [0.16720711037339814] * 1
    + [0.17047749048884808] * 1
    + [0.17461698117974286] * 1
    + [0.1798984163980135] * 1
    + [0.17920355117405806] * 1,
...
if runHCal:
    ...
    calibHcells = CalibrateCaloHitsTool("CalibrateHCal", invSamplingFraction="30.4")
...
createEcalBarrelPositionedCells = CreateCaloCellPositionsFCCee(
    "CreateECalBarrelPositionedCells", OutputLevel=INFO
)
...
createEcalEndcapCells = CreateCaloCells(
...
createEcalEndcapPositionedCells = CreateCaloCellPositionsFCCee(
    "CreateECalEndcapPositionedCells", OutputLevel=INFO
)
...
if runHCal:
    ...
    cellPositionHcalBarrelTool = CellPositionsHCalPhiThetaSegTool(
        "CellPositionsHCalBarrel", readoutName=hcalBarrelReadoutName, OutputLevel=INFO
    )
    hcalBarrelPositionedCellsName = "HCalBarrelPositionedCells"
    createHcalBarrelPositionedCells = CreateCaloCellPositionsFCCee(
        "CreateHcalBarrelPositionedCells", OutputLevel=INFO
    )
...
correctCaloClusters = CorrectCaloClusters(
    "correctCaloClusters",
    ...
    downstreamParameters=[
        [
            -0.0027661744480442195,
            0.006059143775380306,
            0.9788596364251927,
            -1.4951749409378743,
            -0.08491999337012696,
            16.017621428757778,
    ...
    benchmarkParamsApprox=[1.22, 1, 1.04, -0.0019, 25.69, 0.0],
    ...
    benchmarkParametrization=[
        [2.04, 1.0626, 1.0, -3.5, 1.0182, -0.26, 0.0004, 45.9, -5906.11, -129.27, 0.0],
        [],
    ],
    benchmarkFormulas=[
        ["[0]/sqrt(x)+[1]", "[0]", "[0]/x+[1]", "[0]/x+[1]", "[0]+[1]/(x-[2])", "[0]"],
        [],
    ],
    benchmarkEneSwitch=-1.0,
    upstreamCorr=False,
