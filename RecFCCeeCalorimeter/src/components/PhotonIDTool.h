...
private:
   ...
   * @return                  Status code.
   */
  StatusCode readMVAFiles(const std::string& mvaInputsFileName, const std::string& mvaModelFileName);
  ...
  mutable k4FWCore::DataHandle<edm4hep::ClusterCollection> m_outClusters{"outClusters", Gaudi::DataHandle::Writer, this};

  /// Handles for the cluster shower shape metadata to read and to write
  k4FWCore::MetaDataHandle<std::vector<std::string>> m_inShapeParameterHandle{m_inClusters, edm4hep::labels::ShapeParameterNames,
  ...
  Gaudi::Property<std::string> m_mvaInputsFile{this, "mvaInputsFile", {}, "JSON file with the mva inputs"};
