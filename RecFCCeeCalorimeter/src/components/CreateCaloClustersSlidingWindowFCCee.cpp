  ...
  if (m_createClusterCellCollection) {
    ...
    if (tool) {
      ...
      if (IDs.size() == colls.size()) {
        std::string systemIDsName = podio::collMetadataParamName(m_clusters.objKey(), "inputSystemIDs");
        k4FWCore::putParameter (systemIDsName, IDs, this);

        std::string cellCollectionsName = podio::collMetadataParamName(m_clusters.objKey(), "inputCellCollections");
