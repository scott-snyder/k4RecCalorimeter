...
StatusCode CreateCaloCells::execute(const EventContext&) const {
  ...
  if (m_addCrosstalk) {
    ...
    for (const auto& this_cell : m_cellsMap) {
      ...
      auto vec_neighbours = m_crosstalksTool->getNeighboursSpan(this_cellId); // a span of neighbour IDs
      auto vec_crosstalks = m_crosstalksTool->getCrosstalksSpan(this_cellId); // a span of crosstalk coefficients
