 ...
 * (a) get seed cells above threshold t1Please note that cluster collections, build from a different algorithm e.g.
 * sliding window, could be used as well. (b) check if 4 neighbouring cells exist with energy > 2nd topo-cluster
 * threshold (c) if more than one maximum was found, start the splitting.
 * 2. start splitting:
 ...
 * (c) if cell has been identified for two clusters, distance from the centre-of-gravity of thecluster is determined,
 * and the cell gets assigned to the closest cluster.
 * 3. finalisation:
...
public:
   ...
   */
  std::vector<std::pair<uint64_t, uint>> searchForNeighbours(const uint64_t aCellId, uint aClusterID,
                                                             const std::map<uint64_t, int> aCellType,
                                                             std::map<uint64_t, uint>& aClusterOfCell,
                                                             std::map<uint64_t, TLorentzVector> aCellPosition,
                                                             std::map<uint, TLorentzVector>& aClusterPositions) const;
...
private:
  ...
  mutable k4FWCore::DataHandle<edm4hep::ClusterCollection> m_newClusters{"calo/calibClusters", Gaudi::DataHandle::Writer, this};
  // Handle for calo cells (output collection)
  mutable k4FWCore::DataHandle<edm4hep::CalorimeterHitCollection> m_newCells{"calo/calibClusterCells", Gaudi::DataHandle::Writer,
                                                                   this};
