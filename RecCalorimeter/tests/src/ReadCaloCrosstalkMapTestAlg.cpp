...
private:
  ToolHandle<ICaloReadCrosstalkMap> m_tool
    {this, "Tool", "ReadCaloCrosstalkMap", ""};
...
StatusCode ReadCaloCrosstalkMapTestAlg::writeFile(unsigned int ncells,
                                                  const std::string& fileName,
                                                  const k4::recCalo::ICaloIndexer& indexer) const {
  ...
  for (size_t icell = 0; icell < ncells; icell++) {
    ...
    write_neighbours.assign (cellIDs.begin() + ineigh,
                             cellIDs.begin() + ineigh + nneigh);
...
StatusCode ReadCaloCrosstalkMapTestAlg::checkCrosstalk (unsigned int ncells,
                                                        const ICaloReadCrosstalkMap& tool,
                                                        const k4::recCalo::ICaloIndexer& indexer) const
{
  std::span<const CellID> cellIDs = indexer.cellIDs();
  for (size_t icell = 0; icell < ncells; icell++) {
    ...
    if (neighs.size() != nneigh || xtalk.size() != nneigh) {
      error() << "Size mismatch for cell index " << icell << " ID " << cellIDs[icell]
              << " expected " << nneigh << " got "
              << neighs.size() << " " << xtalk.size() << endmsg;
      return StatusCode::FAILURE;
    }
    unsigned ineigh = icell*10 + 100;
    for (unsigned i = 0; i < nneigh; i++) {
      if (neighs[i] != cellIDs[ineigh + i]) {
        error() << "Neighbour mismatch for cell index " << icell << " ID " << cellIDs[icell]
                << " offset " << i
                << " expected " << cellIDs[ineigh+i] << " got "
                << neighs[i] << endmsg;
        return StatusCode::FAILURE;
      }
